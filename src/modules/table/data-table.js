 ...
 */
'use strict';
let html = require('./data-table.es6');
...
class Table extends BaseModule {
    constructor(element = {}, creator, key) {
        ...
        this.element.param = JSON.fix(this.element.param)
        ...
        $html.on("click", "[data-fn=column]", function () {
                ...
                "buttons": [
                    {
                        ...
                        events: {
                            "click": function (d) {
                                $html.find("[data-name=columns]").data("value",result.result());
                                d.close();
                            }
                        }
                    }
            ...
            $div.empty().append(
                (result = table.generator(
                    $html.find("[data-name=columns]").data("value") || context.element.param.columns
                )).view
            )
        });
